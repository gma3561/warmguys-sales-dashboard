...
const SHEET_CONFIG = {
  ...
  'Warmguys_DashBoaod_Rawdata': {
    tableName: 'fuel_transactions',
    companyType: 'fuel',
    startRow: 4  // 데이터가 4행부터 시작
  }
};
...
function readSheetData(sheet, config) {
  ...
  for (let i = startRow + 1; i < values.length; i++) {
    ...
    headers.forEach((header, index) => {
      ...
      // 날짜 처리
      if (header && (header.toLowerCase().includes('date') || header === '날짜' || header === '송금일자' || header === '입하일자')) {
    ...
    const dateField = config && config.companyType === 'fuel' ? '송금일자' : headers[0];
  ...
  return rawData.map(row => {
    ...
    if (companyType === 'apgujeong') {
      ...
      return {
        ...
        // 원가 정보 (A-F)
        purchase_supplier: row[headers[1]] || '', // B열: 발주처
        fuel_type: row[headers[2]] || '', // C열: 유종
        purchase_unit_price: parseFloat(row[headers[3]] || 0), // D열: 단가
        purchase_volume: parseInt(row[headers[4]] || 0), // E열: 리터
        purchase_total_amount: parseInt(row[headers[5]] || 0), // F열: 합계
        ...
        delivery_destination: row[headers[12]] || '', // M열: 납품처
        sale_unit_price: parseFloat(row[headers[14]] || 0), // O열: 단가
        sale_volume: parseInt(row[headers[15]] || 0), // P열: 리터
        sale_total_amount: parseInt(row[headers[16]] || 0), // Q열: 합계
...
function testDataTransformation() {
  ...
  try {
    ...
    // 시트 데이터 읽기
    const data = readSheetData(sheet, SHEET_CONFIG['압구정곱창 압구정점']);
...
function testMRSData() {
  ...
  try {
    ...
    const data = readSheetData(sheet, SHEET_CONFIG['엠알에스']);
...
function testFuelData() {
  ...
  try {
    const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = spreadsheet.getSheetByName('Warmguys_DashBoaod_Rawdata');
    ...
    const data = readSheetData(sheet, SHEET_CONFIG['Warmguys_DashBoaod_Rawdata']);
    console.log('연료 데이터 개수:', data.length);
    
    if (data.length > 0) {
      console.log('연료 첫 번째 원본:', JSON.stringify(data[0], null, 2));
      
      const transformed = transformDataForSupabase(data.slice(0, 2), SHEET_CONFIG['Warmguys_DashBoaod_Rawdata']);
...
function testConnectionStatus() {
  ...
  // 설정된 시트들과 비교
  const expectedSheets = ['압구정곱창 압구정점', '압구정곱창 가로수점', '엠알에스', 'Warmguys_DashBoaod_Rawdata'];
  const results = {};
